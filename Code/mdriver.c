...
static trace_t *read_trace(char *tracedir, char *filename)
{
	...
	int temp;		//created to suppress warnings
    temp = fscanf(tracefile, "%u", &(trace->sugg_heapsize)); /* not used */
    temp = fscanf(tracefile, "%u", &(trace->num_ids));     
    temp = fscanf(tracefile, "%u", &(trace->num_ops));     
    temp = fscanf(tracefile, "%u", &(trace->weight));        /* not used */
	...
	switch(type[0]) {
	case 'a':
	    temp = fscanf(tracefile, "%u %u", &index, &size);
	    if(temp == -1)
		{}
	...
	case 'r':
	    ...
	    break;
	case 'f':
	    temp = fscanf(tracefile, "%ud", &index);
	    if(temp == -1)
                {}
